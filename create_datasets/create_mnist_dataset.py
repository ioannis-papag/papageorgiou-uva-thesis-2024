...
def main(args):
    ...
    isTestSetUnique = args.unique_test
    num_test_set = args.test_length
    ...
    with h5py.File(file_path_img, 'w') as img_hf, h5py.File(file_path_box, 'w') as box_hf:
        for k in tqdm(range(num_steps)):
            ...
            img, clothes, obj_coords = env.render(rng_gen=rng, mnist=True) #Render the image

            #Keep track of all the clothes indices used
            clothes_list.append(clothes.copy())

            if isTestSetUnique:
                #If every image of the test set should be unique, keep track of all the seen states in the training set
        ...
        labels_df.to_csv(f'dataset_mnist/labels.csv', index=False, sep=';')
        
        #Save all the clothes indices to a csv file
        clothes_df = pd.DataFrame(clothes_list)
        clothes_df.to_csv(f'dataset_mnist/clothes.csv', index=False, sep=';')



    #Create output directories
    output_dir = f'dataset_mnist_test/images/'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)


    output_dir_img = f'dataset_mnist_test/imgs'
    if not os.path.exists(output_dir_img):
        os.makedirs(output_dir_img)
    file_path_img = os.path.join(output_dir_img, 'imgs.h5')


    output_dir_box = f'dataset_mnist_test/boxes'
    if not os.path.exists(output_dir_box):
        os.makedirs(output_dir_box)
    file_path_box = os.path.join(output_dir_box, 'boxes.h5')
    ...
    labels = []
    clothes_list = []
    if isTestSetUnique and len(seen_predicates) == 7000:
        seen_predicates = []
    ...
    with h5py.File(file_path_img, 'w') as img_hf, h5py.File(file_path_box, 'w') as box_hf:
        for k in tqdm(range(num_test_set)):
            if k > 0:
                ...
                #While the state has been already visited, take a new action
                if isTestSetUnique:
                    while obs.predicates in seen_predicates:
                        action = env.action_space.sample(obs)
                        obs, reward, done, info = env.step(action)
                else:
                    action = env.action_space.sample(obs)
        ...
        #Save all the labels/targets as a csv file
        labels_df = pd.DataFrame(labels, columns=literals)
        labels_df.to_csv(f'dataset_mnist_test/labels.csv', index=False, sep=';')

        #Save all the clothes indices to a csv file
        clothes_df = pd.DataFrame(clothes_list)
        clothes_df.to_csv(f'dataset_mnist_test/clothes.csv', index=False, sep=';')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--length", type=int, help="Length of the dataset to be generated", required=False, default=50000)
    parser.add_argument("-l", "--unique_test", type=bool, help="If True, all the samples of the test set are of unique layouts", default="True")
    parser.add_argument("-tn", "--test_length", type=int, help="Length of the test set to be generated", required=False, default=10000)
